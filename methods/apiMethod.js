...
class ApiGet extends ApiMethod {
  ...
  setQuery() {
    ...
    if (condition) {
      this.query = `SELECT ${fields} FROM ${getTable(
        this.method.getPath(2)
      )} WHERE ${condition}`;
    } else {
...
class ApiPost extends ApiMethod {
  ...
  async setQuery() {
    ...
    this.query = `INSERT INTO ${getTable(
      this.method.getPath(2)
...
class ApiPut extends ApiMethod {
  ...
  async setQuery() {
    ...
    this.query = `UPDATE ${getTable(
      this.method.getPath(2)
    )} SET ${fieldstr} WHERE ${condition}`;
  }
}
...
class ApiDelete extends ApiMethod {
  ...
  async setQuery() {
    const condition = this.setConditions();
    this.query = `DELETE FROM ${getTable(
      this.method.getPath(2)
    )} WHERE ${condition}`;
  }
}
